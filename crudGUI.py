...
class UpdateGUI:
    def __init__(self, master, input_file):
        ...
        self.l_top_frame = tkinter.Frame(self.update)
        self.ll_top_frame = tkinter.Frame(self.update)
        ...
        self.update_label2 = tkinter.Label(self.l_top_frame, text='Enter new email: \t')
        self.update_entry2 = tkinter.Entry(self.l_top_frame, width=15)
        self.update_label3 = tkinter.Label(self.ll_top_frame, text='Enter customers name: \t')
        self.update_entry3 = tkinter.Entry(self.ll_top_frame, width=15)

        self.update_label.pack(side='left')
        self.update_entry.pack(side='left')
        self.update_label2.pack(side='left')
        self.update_entry2.pack(side='left')
        self.update_label3.pack(side='left')
        self.update_entry3.pack(side='left')
        ...
        self.top_frame.pack()
        self.l_top_frame.pack()
        self.ll_top_frame.pack()
        self.middle_frame.pack()
        self.bottom_frame.pack()
    ...
    def update_value(self):  # Don't let shadow error happen
        ...
        original_name = self.update_entry.get()
        name_change = self.update_entry3.get()
        if original_name in customers:
            customers[change_name] = name_change
            del customers[original_name]
            result = customers.get(change_name, 'Customer info updated')
            self.value.set(result)
        else:
            ex_result = customers.get(original_name, 'Does not Exist')
...
class DeleteGUI:
    def __init__(self, master, input_file):
        ...
        self.delete_button = tkinter.Button(self.bottom_frame, text='Delete', command=self.delete_value)  # shadow error
        self.back_button = tkinter.Button(self.bottom_frame, text='Main Menu', command=self.back)
